	...
	void getDegrees(Network n) {
		...
		for (NetworkNode node : l) {
			if (node instanceof Neuron) {
				int degree = ((Neuron) node).getNeuronConnections().size();
				if (((Neuron) node).getType() == Type.RS) {
				...
				if (((Neuron) node).getType() == Type.FS) {
				...
				if (((Neuron) node).getType() == Type.LTS) {
				...
				if (((Neuron) node).getType() == Type.IB) {
